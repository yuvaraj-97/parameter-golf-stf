...
def render_validation_ladder(completed: list[Variant]) -> str:
    ...
    <div class="ladder-card" id="validation-ladder">
      <div>
        ...
        <p class="note">500-step runs are treated as smoke tests. The 10k rows are the real current MLP-skip quality signal. Quality delta is shown against the fixed active-0.75 MLP-skip run at the same iteration count when available.</p>
      </div>
...
def load_legacy_analysis_reports(root: Path = Path("My_approch") / "analysis") -> list[dict[str, object]]:
    reports: list[dict[str, object]] = []
    if not root.exists():
        return reports
    for path in sorted(root.glob("*_branch_compare_data.json")):
        try:
...
def render_legacy_analysis_coverage() -> str:
    reports = load_legacy_analysis_reports()
    if not reports:
        return ""
    ...
    for report in reports:
        ...
        shard_filter = str(report.get("shardFilter") or "preferred")
        for record in report.get("records", []):
            if not isinstance(record, dict):
                continue
            branch = str(record.get("branch") or "")
            if branch and branch not in branch_meta:
                branch_meta[branch] = {
                    "family": record.get("family") or "-",
                    "decision": record.get("decision") or "-",
                    "frozen_path": record.get("frozen_path") or "-",
                    "control": record.get("control") or "-",
                    "summary": record.get("summary") or "",
                }
            row = dict(record)
            row["_iterations"] = iterations
            row["_shard_filter"] = shard_filter
            rows.append(row)
    ...
    best_rows = sorted(
        ok_rows,
        key=lambda row: (
            as_float(row.get("roundtrip_val_bpb")) or as_float(row.get("final_val_bpb")) or float("inf"),
            int(row.get("_iterations") or 0),
            str(row.get("branch") or ""),
        ),
    )[:10]
    missing_rows = [row for row in rows if row.get("status") != "ok"]

    coverage_body = []
    for row in sorted(
        rows,
        key=lambda item: (
            int(item.get("_iterations") or 0),
            str(item.get("_shard_filter") or ""),
            str(item.get("branch") or ""),
        ),
    ):
        status = str(row.get("status") or "missing")
        status_class = "done" if status == "ok" else "blocked"
        branch = str(row.get("branch") or "-")
        coverage_body.append(
            "<tr>"
            f"<td>{int(row.get('_iterations') or 0):,}</td>"
            f"<td>{html.escape(str(row.get('_shard_filter') or '-'))}</td>"
            f"<td>{html.escape(branch)}</td>"
            f"<td><span class=\"status {status_class}\">{html.escape(status)}</span></td>"
            f"<td>{fmt(row.get('best_step') if isinstance(row.get('best_step'), int) else None)}</td>"
            f"<td>{fmt(as_float(row.get('best_val_bpb')))}</td>"
            f"<td>{fmt(as_float(row.get('final_val_bpb')))}</td>"
            f"<td>{fmt(as_float(row.get('roundtrip_val_bpb')))}</td>"
            f"<td>{fmt(as_float(row.get('step_avg_ms')), 1)}ms</td>"
            f"<td>{fmt(as_float(row.get('gpu_util_avg')), 1)}%</td>"
            f"<td>{fmt(as_float(row.get('mem_peak_gb')), 2)}GB</td>"
            f"<td>{html.escape(str(row.get('artifact_margin_bytes') or '-'))}</td>"
            "</tr>"
        )
    ...
    for row in best_rows:
        best_body.append(
            ...
            f"<td>{fmt(as_float(row.get('final_val_bpb')))}</td>"
            f"<td>{fmt(as_float(row.get('step_avg_ms')), 1)}ms</td>"
            f"<td>{html.escape(str(row.get('train_shards') or '-'))}</td>"
            f"<td>{html.escape(str(row.get('summary') or ''))}</td>"
            "</tr>"
        )

    meta_cards = []
    for branch, meta in sorted(branch_meta.items()):
        meta_cards.append(
            "<article class=\"branch-chip\">"
            "<span class=\"status archive\">legacy</span>"
            f"<strong>{html.escape(branch)}</strong>"
            f"<small>{html.escape(str(meta.get('family') or '-'))} | {html.escape(str(meta.get('decision') or '-'))} | {html.escape(str(meta.get('control') or '-'))}</small>"
            f"<small>{html.escape(str(meta.get('summary') or ''))}</small>"
            "</article>"
        )

    return f"""
    <section class="compare-card" id="legacy-coverage">
      <p class="eyebrow">Imported branch-comparison bundle, embedded</p>
      <h2>Legacy Iteration Coverage</h2>
      <p class="note">This folds the old branch-comparison JSON directly into the main report. These are legacy control/STF branch-comparison rows, not the same run family as the current MLP-skip validation ladder above. The 5k shard-80 baseline-repro score is valid for that old control run, but it should not be read as an adaptive MLP-skip target.</p>
      <div class="metrics">
        <div class="metric"><strong>{len(reports)}</strong><span>legacy data bundles</span></div>
        <div class="metric"><strong>{len(ok_rows)}</strong><span>completed old rows</span></div>
        <div class="metric warn"><strong>{len(missing_rows)}</strong><span>explicit old gaps</span></div>
        <div class="metric"><strong>{len(branch_meta)}</strong><span>branch definitions</span></div>
      </div>
      <div class="table-wrap">
        <table>
          <thead><tr><th>Iterations</th><th>Shard View</th><th>Branch</th><th>Status</th><th>Best Step</th><th>Best BPB</th><th>Final BPB</th><th>Roundtrip BPB</th><th>Avg Iter</th><th>GPU Avg</th><th>VRAM Peak</th><th>16MB Margin</th></tr></thead>
          <tbody>{''.join(coverage_body)}</tbody>
        </table>
      </div>
      <div class="section-title">
        <h2>Best Legacy Rows</h2>
        <p class="note">Ranked by roundtrip BPB when available, otherwise final validation BPB. Legacy rows are useful sanity anchors, but direct budget tradeoffs should use the current fixed/adaptive MLP-skip rows at matching iteration counts.</p>
      </div>
      <div class="table-wrap">
        <table>
          <thead><tr><th>Iterations</th><th>Shard View</th><th>Branch</th><th>Roundtrip BPB</th><th>Final BPB</th><th>Avg Iter</th><th>Train Shards</th><th>Branch Meaning</th></tr></thead>
          <tbody>{''.join(best_body)}</tbody>
        </table>
      </div>
...
def parse_run_log_summary(path: Path) -> dict[str, object]:
    summary: dict[str, object] = {
        ...
        "score_fn": "-",
        "run_id": path.parent.name if path.name in {"console.log", "train.log"} else path.stem,
        "last_val_step": None,
        "max_steps": None,
        "last_val_bpb": None,
        "best_val_bpb": None,
        ...
        with path.open("r", encoding="utf-8", errors="replace") as handle:
            for raw in handle:
                ...
                if line.startswith("branch="):
                    ...
                    continue
                if line.startswith("score_fn="):
                    summary["score_fn"] = line.split("=", 1)[1].strip()
                    continue
                if line.startswith("run_id="):
                ...
                if line.startswith("- STF score function:"):
                    summary["score_fn"] = line.split(":", 1)[1].strip()
                    continue
                if "stf_score_fn:" in line:
                    match = re.search(r"\bstf_score_fn:([A-Za-z0-9_./-]+)", line)
                    if match:
                        summary["score_fn"] = match.group(1)

                val_match = VAL_RE.search(line)
                if val_match:
                    step = int(val_match.group(1))
                    max_steps = int(val_match.group(2))
                    val_bpb = parse_float(val_match.group(4))
                    summary["last_val_step"] = step
                    summary["max_steps"] = max_steps
                    summary["last_val_bpb"] = val_bpb
                ...
                lowered = line.lower()
                if "variant failed" in lowered or "traceback " in lowered or "outofmemory" in lowered or "out of memory" in lowered:
                    summary["status"] = "failed"
    except OSError:
        summary["status"] = "unreadable"

    if summary["status"] == "seen" and summary.get("last_val_bpb") is not None:
        summary["status"] = "partial"
    return summary


def merge_run_summaries(rows: list[dict[str, object]]) -> list[dict[str, object]]:
    grouped: dict[str, dict[str, object]] = {}
    for row in rows:
        key = str(row.get("group") or row.get("path"))
        current = grouped.get(key)
        if current is None:
            merged = dict(row)
            merged["files"] = Path(str(row["path"])).name
            grouped[key] = merged
            continue
        current["files"] = ", ".join(sorted(set(str(current.get("files", "")).split(", ") + [Path(str(row["path"])).name])))
        for field_name in ["branch", "score_fn", "run_id"]:
            if current.get(field_name) in {None, "-", ""} and row.get(field_name) not in {None, "-", ""}:
                current[field_name] = row[field_name]
        for field_name in ["last_val_step", "max_steps", "last_val_bpb", "best_val_bpb", "final_bpb", "last_step_avg_ms"]:
            if row.get(field_name) is not None:
                current[field_name] = row[field_name]
        status_rank = {"unreadable": 0, "seen": 1, "partial": 2, "failed": 3, "completed": 4}
        if status_rank.get(str(row.get("status")), 0) > status_rank.get(str(current.get("status")), 0):
            current["status"] = row["status"]
    return sorted(grouped.values(), key=lambda item: (str(item.get("branch")), str(item.get("group"))))
...
def render_all_run_inventory() -> str:
        ...
        rows.append(
            {
                ...
                "run_id": variant.run_id or (source_path.parent.name if source_path.name in {"console.log", "train.log"} else source_path.stem),
    ...
    generic_paths = [
        path
        for path in paths
        if str(path) not in parsed_sources and run_group_key(path) not in parsed_groups
    ]
    rows.extend(merge_run_summaries([parse_run_log_summary(path) for path in generic_paths]))
    ...
    for row in rows:
        ...
        body.append(
            ...
            "</div>"
            "<div class=\"run-paths\">"
            f"<span>{html.escape(str(row.get('files') or '-'))}</span>"
            f"<code>{html.escape(str(row.get('group') or row.get('path') or '-'))}</code>"
            "</div>"
            "</article>"
        )
    ...
    <section class="compare-card" id="all-run-inventory">
      ...
      <p class="note">This is the coverage ledger for owned STF/Vast artifacts: every top-level Vast log and archived Vast experiment console/train log discovered in the repo is listed here. Rows marked partial/seen may not have STF metadata, but they are still noted so old runs do not disappear from the assembled report. Separate non-STF archive material is intentionally excluded because it is not part of this branch set.</p>
      <div class="metrics">
...
def branch_description(branch: str) -> str:
    descriptions = {
        "main": "Integration branch used to assemble the report and shared scripts.",
        "baseline-repro": "OpenAI-style direct baseline context. No STF routing; use for environment and quality reference when matching metrics are available.",
        "codex/stf-true-skip-foundation": "Foundation branch for pre-block active masks, predictor plumbing, and compute telemetry.",
        "codex/stf-mlp-skip-runner": "Validated true-skip MLP path. Gathers active token rows through MLP and scatters them back, so logical freeze becomes real MLP compute savings.",
        ...
        "codex/stf-query-sparse-attn": "Attempted sparse attention plus MLP true-skip path. Current smoke attempts are failed/blocked, so this needs redesign before more pod time.",
        "stf-minimal": "Plain STF experiment: hard-threshold low-change tokens and carry previous hidden state.",
        "stf-learned-gate": "Learns a continuous gate instead of relying only on a fixed threshold. Good quality in some runs, but gate-open proxy often means little true freezing.",
        "stf-learned-gate-telemetry": "Telemetry variant of learned-gate that exposes compute reality; useful for separating logical gates from actual skipped work.",
        "stf-soft-freeze": "Soft-freeze path that blends in a fraction of the new state for frozen tokens to reduce hard-freeze brittleness.",
        ...
        "stf-reactivation": "Periodically reopens tokens so early freeze decisions are not permanent.",
        "stf-budget-regularization": "Adjusts threshold pressure to maintain an active-token budget.",
        "stf-recurrence": "Uses a recurrence-like old/new blend for frozen tokens.",
        "stf-recurrence-telemetry": "Telemetry recurrence variant showing compute reality for recurrent frozen-state paths.",
        "stf-quantization": "Archived experiment testing whether frozen tokens can carry a cheaper quantized previous state.",
    }
    return descriptions.get(branch, branch_status(branch)[1])
...
def render_branch_compare_picker(completed: list[Variant]) -> str:
    ...
    payload = html.escape(json.dumps(branch_records, separators=(",", ":")), quote=False)

    options = "".join(
        f'<option value="{html.escape(record["branch"])}">{html.escape(record["branch"])}</option>'
        for record in branch_records
    )
    default_a = html.escape(branch_records[0]["branch"])
    default_b = html.escape(branch_records[1]["branch"] if len(branch_records) > 1 else branch_records[0]["branch"])
    ...
    <div class="compare-card" id="branch-explorer">
      <div class="compare-copy">
        ...
        <p class="note">Pick any two branches. The chart draws every validation curve available for those branches, and the panels below summarize what each branch was trying to prove plus its best metrics.</p>
      </div>
...
<head>
  ...
  <style>
    ...
    * {{ box-sizing: border-box; letter-spacing: 0 !important; }}
    ...
    .run-row {{
      display: grid;
      grid-template-columns: minmax(220px, .85fr) minmax(360px, 1fr) minmax(320px, 1.15fr);
      gap: 14px;
      align-items: center;
      border: 1px solid var(--line);
      border-radius: 16px;
    ...
    .status.main {{ color: #061208; background: var(--active); }}
  ...
  <script>
    ...
    (function() {{
      ...
      function renderDelta(first, second) {{
        ...
        const qualityClass = Number.isFinite(bpbDelta) ? (bpbDelta <= 0 ? "good" : "bad") : "neutral";
        const speedClass = Number.isFinite(speedDelta) ? (speedDelta <= 0 ? "good" : "bad") : "neutral";
        const skipClass = Number.isFinite(skipDelta) ? (skipDelta >= 0 ? "good" : "bad") : "neutral";
        deltaDetail.innerHTML = `<span class="status branch">best-to-best</span>
          ...
          <div class="branch-stats">
            <div class="branch-stat"><strong class="${{qualityClass}}">${{fmt(bpbDelta)}}</strong><span>BPB delta, lower is better</span></div>
            <div class="branch-stat"><strong class="${{speedClass}}">${{pctValue(speedDelta)}}</strong><span>iteration-time delta, lower is faster</span></div>
            <div class="branch-stat"><strong class="${{skipClass}}">${{pctValue(skipDelta)}}</strong><span>actual-skip delta, higher is more skipped compute</span></div>
      ...
      function renderChart(selected) {{
        ...
        selected.forEach((branch, branchIndex) => {{
          (branch.variants || []).forEach((variant, variantIndex) => {{
            const points = (variant.history || [])
              .map((point) => ({{ step: Number(point.step), bpb: Number(point.bpb) }}))
              .filter((point) => Number.isFinite(point.step) && Number.isFinite(point.bpb));
            if (points.length) {{
              series.push({{
                branch: branch.branch,
                formula: variant.formula,
                color: colors[branchIndex % colors.length],
                opacity: variantIndex === 0 ? 0.95 : 0.42,
                points
              }});
            }}
          }});
